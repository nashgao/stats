...
public class SMC {
    ...
    /// Serializes every use of `conn`: readers run on multiple queues,
    /// and reconnect()/close swap the connection out from under them.
    ...
    /// Open the AppleSMC connection. The caller must hold `connLock`.
    private func openConnection() -> Bool {
        var iterator: io_iterator_t = 0
        let device: io_object_t

        let matchingDictionary: CFMutableDictionary = IOServiceMatching("AppleSMC")
        var result = IOServiceGetMatchingServices(kIOMainPortDefault, matchingDictionary, &iterator)
        ...
        result = IOServiceOpen(device, mach_task_self_, 0, &self.conn)
    ...
    /// Drop and reopen the AppleSMC connection. Safe to call repeatedly
    /// and while already connected: sleep/wake can stale the connection
    /// opened at launch, so the wake handler refreshes it proactively.
    public func reconnect() {
        self.connLock.lock()
        defer { self.connLock.unlock() }
        if self.conn != 0 {
            _ = IOServiceClose(self.conn) // ignore close errors
            self.conn = 0
        }
        _ = self.openConnection()
    }
    ...
    public func close() -> kern_return_t {
        self.connLock.lock()
        defer { self.connLock.unlock() }
        guard self.conn != 0 else { return kIOReturnSuccess }
        let result = IOServiceClose(self.conn)
        self.conn = 0
        return result
    }
    ...
    /// Errors that mean the AppleSMC connection itself is dead or stale
    /// (the io_connect_t opened at launch does not survive sleep/wake),
    /// as opposed to a failed key read — these warrant one reconnect.
    /// (kIOReturnServiceNotFound/kIOReturnBadPort from the original list
    /// do not exist in the macOS SDK.)
    private static func isConnectionError(_ result: kern_return_t) -> Bool {
        result == kIOReturnNoDevice || result == kIOReturnIPCError
            || result == kIOReturnNotReady || result == kIOReturnInvalid
    }
    ...
    private func read(_ value: UnsafeMutablePointer<SMCVal_t>) -> kern_return_t {
        ...
        result = call(SMCKeys.kernelIndex.rawValue, input: &input, output: &output)
        if Self.isConnectionError(result) {
            // Post-sleep freeze: the connection opened at launch can go
            // stale across sleep/wake, so reopen it and retry the call
            // once; a failed retry returns the error as before.
            self.reconnect()
            result = call(SMCKeys.kernelIndex.rawValue, input: &input, output: &output)
        }
        if result != kIOReturnSuccess {
    ...
    private func call(_ index: UInt8, input: inout SMCKeyData_t, output: inout SMCKeyData_t) -> kern_return_t {
        ...
        self.connLock.lock()
        let result = IOConnectCallStructMethod(conn, UInt32(index), &input, inputSize, &output, &outputSize)
        self.connLock.unlock()
        return result
    }
}
